...
def validate_info_across_pages(pdf_path):
    ...
    return {
        ...
        "sheet_numbers": {
            "found": sheet_numbers,
            "missing": sequence_analysis.get("prefix_groups", [{}])[0].get("missing_sheet_numbers", []),
            "analysis": sequence_analysis.get("analysis", "")
        },
        "summary": f"Found {len(discrepancies)} discrepancies across {len(pages_text)} pages. {len(sheet_numbers['missing'], [])} sheet numbers missing in sequence."
    }
